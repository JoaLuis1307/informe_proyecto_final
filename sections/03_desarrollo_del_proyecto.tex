...
La Figura~\ref{fig:arquitectura_lora_p2p} presenta la arquitectura de comunicación implementada.

\begin{figure}[H]
    \centering
    \resizebox{0.94\textwidth}{!}{
    \begin{tikzpicture}[
        node distance=1.15cm and 3.6cm,
        nodo/.style={
            rectangle,
            rounded corners=3pt,
            draw=black,
    ...
    \node[nodo] (nodo1) {
        \textbf{Nodo IoT 1}\\
        ID: N01\\
        Sensores + ESP32 + SX1278
    };

    \node[nodo, below=of nodo1] (nodo2) {
        \textbf{Nodo IoT 2}\\
        ID: N02\\
        Sensores + ESP32 + SX1278
    };

    \node[nodo, below=of nodo2] (nodon) {
        \textbf{Nodo IoT N}\\
        ID: N\ldots\\
        Sensores + ESP32 + SX1278
    };
    ...
    % Gateway
    \node[gateway, right=of nodo2] (gateway) {
        \textbf{Gateway LoRa-WiFi}\\[1mm]
        ESP32-S3 + SX1278\\
    ...
    % Servidor
    \node[servidor, right=3.5cm of gateway] (servidor) {
        \textbf{Servidor central}\\[1mm]
        Broker MQTT\\
    ...
    \draw[flecha]
        (nodo1.east) --
        node[above, etiqueta]{LoRa P2P}
        (gateway.west |- nodo1.east);
    ...
    \draw[flecha]
        (nodon.east) --
        node[above, etiqueta]{LoRa P2P}
        (gateway.west |- nodon.east);

    % Enlace IP
    \draw[flecha]
        (gateway.east) --
        node[above, etiqueta]{WiFi / MQTT}
        (servidor.west);

    % Retorno opcional de configuración
    \draw[retorno]
        ([yshift=-5mm]gateway.east) --
        node[below, etiqueta]{Configuración o confirmación opcional}
        ([yshift=-5mm]servidor.west);

    \end{tikzpicture}
    }

    \caption{Arquitectura de comunicación LoRa P2P entre nodos, gateway y servidor central.}
    \label{fig:arquitectura_lora_p2p}
\end{figure}
